...
In the Community Edition the log is still written and can be exploited outside the interface: [forward it to a SIEM](/hosting/maintenance/diagnostics/forward-action-logs/) or query the database directly, and control its growth with the [purge command](/hosting/useful-commands/#action-logs-purge).

## What is recorded

Passbolt writes one row per authenticated API call, which the audit log then turns into activity entries: resources created and updated, secrets read and updated, permission changes, folders created and updated, and user accounts created, updated and deleted.
