...
What the user sees depends on the client:

- On the API and the mobile and desktop applications, the refresh token is revoked and the response carries "You have been logged out due to too many failed attempts."
- In the browser, no message is shown: the user simply lands back on the sign-in screen, with no explanation.
