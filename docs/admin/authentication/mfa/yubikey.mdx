---
title: Configure Passbolt to Use Yubikey
...
To enable YubiKey via the interface, select &#9881; > **Organisation settings** > **Multi Factor Authentication** in the top right corner.

Subsequently, enable the "Yubikey" provider by moving the adjacent toggle to the on position and input the information provided by YubiCloud at the previous step. Ensure you save these modifications to activate the provider.
...
If you are [using docker](/hosting/install/ce/docker/), you can set these environment variables to enable YubiKey for your organisation.
...
To setup YubiKey as multi-factor authentication method, please refer to our [dedicated user guide](/user/advanced-features/mfa/yubikey/).
