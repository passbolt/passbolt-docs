...
:::warning[Important]
Passbolt will request a Google API for authorisation, if you have firewall rules setup, you have to allow your server to request the accounts.google.com domain.
:::

## How does it work?

In short Passbolt SSO leverages Google OAuth2/OpenID on top of the existing [challenge-based authentication](/development/authentication/). The user by logging in Google unlocks a key stored server side needed to decrypt the secret key passphrase twice encrypted with a non-extractable symetric key stored in the browser extension local storage client side.
...
In the new project screen, you will be prompted to enter a project name, you can edit the project ID or accept the default one, select an organisation and the location. After that, click on "Create" button to create the project, it should appear in your project list as shown above.
...
:::caution[Please note]
As the name suggests, the "Internal" type app will only be available to users within your organisation. However selecting "External" might work, we do not recommend it to use with Passbolt as it can let any user with a valid google account can sign-in to Passbolt.
:::
...
The Authorized redirect URI must be `https://your-domain.com/sso/google/redirect`, and it must match your instance full base URL exactly, including the https scheme. You can copy it from the read-only Redirect URL field on the passbolt SSO settings page. Google accepts any valid URL here, but any other value makes the sign-in fail with a redirect_uri_mismatch error.
:::
...
After that, a dialog will open with a "Sign in with Google" button, click on it. A popup will open asking you to perform the authentication with Google, once the authentication is successful you can save the settings. Once the settings have been saved, you can log out and you should see an SSO option.
