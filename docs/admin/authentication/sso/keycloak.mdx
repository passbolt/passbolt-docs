...
Passbolt relies on the Identity Provider (IdP) to authenticate users and to assert their identity. Users are matched to existing Passbolt accounts by their email address.

Only use an IdP that guarantees users can authenticate only as email addresses they legitimately own and that you administer. If an IdP allows users to self-assign or modify unverified email addresses, an attacker may be able to impersonate another user.

Before enabling SSO, verify that your IdP:
- verifies ownership of user email addresses;
- prevents users from impersonating other email identities;
- provides unique and trustworthy identity assertions.
...
3. Select **"OpenID"** from the available providers
