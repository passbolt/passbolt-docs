...
import AccountRecoveryNote from "/docs/admin/_includes/sso/account-recovery-note.mdx";
...
:::danger
Do not map users to untrusted attributes

The default Microsoft Entra ID configuration maps users by their verified email address. Changing the account mapping to another claim should only be done if you fully understand the security implications.

Never map users using attributes that are not guaranteed to uniquely and permanently identify the user, or that users can manipulate themselves without verification.

Failure to meet these requirements may result in security issues.
:::
...
In short Passbolt SSO leverages Entra ID OAuth2/OpenID on top of the existing [challenge-based authentication](/development/authentication/). The user by logging in Microsoft unlocks a key stored server side needed to decrypt the secret key passphrase twice encrypted with a non-extractable symetric key stored in the browser extension local storage client side.
...
* Select the supported account type you desire. "Accounts in this organisational directory only" is a good default.
...
Azure secrets expire after 6, 12, or 24 months (Microsoft recommends 6 months). When expired, SSO login stops working.

**passbolt automatically notifies all administrators:**
- 10 days before the secret expires
- Immediately if SSO login fails due to an invalid secret

**To rotate the secret:** Create a new secret in Entra ID, update the value and expiry in Passbolt (Administration > Single Sign On), then test SSO login.
:::

:::tip[If SSO stops working]
Don't panic! Administrators can bypass SSO by logging in with their passphrase at `/auth/login`. From there, update the secret in Administration > Single Sign On.
:::
...
### Select the directory endpoint

The first field of the passbolt form, "Login URL", is a dropdown listing the supported Microsoft directory endpoints:

* `https://login.microsoftonline.com`, the global endpoint, selected by default
* `https://login.microsoftonline.us`, for US Government tenants
* `https://login.partner.microsoftonline.cn`, for Azure China tenants

Keep the default unless your tenant is hosted in one of these sovereign clouds; in that case, select the endpoint matching your tenant.
