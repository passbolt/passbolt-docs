...
- The configured provider supports it: Microsoft Entra ID and Google do, the generic OpenID and AD FS providers do not.

## How to enable or disable it

There is no toggle in the administration interface. The feature is enabled by default as soon as an eligible SSO provider is active, and can be switched off with an environment variable:
...
**Step 1.** Open the passbolt instance in the new browser. The welcome screen shows "Welcome back!" with the message "Your browser is not configured to work with this passbolt instance. Please authenticate with the Single Sign-On provider to continue." and a "Sign in with Microsoft" or "Sign in with Google" button.
