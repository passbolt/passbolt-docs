...
- **RSA 3072 or 4096**: choose RSA only when a constraint leaves no other option, in practice a Red Hat system running with FIPS mode enabled, where ECC is not supported.

:::warning[Prefer curve with encrypted metadata]
With encrypted metadata, the default on new instances, every operation carries additional encryption and signature work on top of the secrets themselves. We recommend keeping the curve default and not switching to RSA. The one fair reason to choose RSA is a Red Hat system running with FIPS mode enabled, where ECC is not supported; even then, you may encounter performance issues as your vault grows.
:::
