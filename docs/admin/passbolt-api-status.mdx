...
<div style={{display: "flex", gap: "1rem"}}>
 ...
 Since Passbolt v4.6, it is now possible for both Passbolt PRO and Passbolt CE to run the [healthCheck](/hosting/maintenance/diagnostics/collect-info/#healthcheck) from the user interface in *Administration > Passbolt API Status*.
...
You will see on the right side that we've introduced a colour-coded legend.
- Green: OK ✅
...
#### PHP Version
It helps to ensure you are not running an unsupported PHP version. It should be green if you are running PHP 8.2 or higher, the minimum version required to run passbolt. Older versions, including PHP 7.4, fail this check with "PHP version is too low, passbolt need PHP 8.2 or higher."

If your operating system cannot provide PHP 8.2 or higher, consider [migrating to a new server](/hosting/migrate/server/). A second, warning-level line announces the next minimum PHP version whenever passbolt plans to raise it.
...
#### NTP
The server requires accurate system time for cryptographic operations and security measures. If the system clock is not synchronised, it may lead to authentication errors, expiration issues, and other unexpected behaviour.

If the check fails, it means that NTP could be inactive, and your system clock may not be synchronised. You can take a look at our [dedicated documentation](/hosting/ntp/) for that.
### Config Files
:::tip[PRO TIP]
...
#### Debug mode
We highly not recommend to use the debug mode on a prod or pre-prod environment. If you see **Debug mode is on**, update the configuration file in */etc/passbolt/passbolt.php* and set debug to false, or set the `DEBUG` environment variable to false if you are on Docker or Helm.
...
It helps to ensure that the **security.salt** value used for the hash is unique and correctly configured. If you have any error you will need to edit the `security.salt` in */etc/passbolt/app.php*, or set the `SECURITY_SALT` environment variable if you are on Docker or Helm.
...
- Edit `App.fullBaseUrl` in */etc/passbolt/passbolt.php*, or the `APP_FULL_BASE_URL` environment variable if you are on Docker or Helm
- Select a valid domain name as defined by section 2.3.1 of [RFC 1035](https://www.ietf.org/rfc/rfc1035.txt)
...
- `App.fullBaseUrl` is correct inside */etc/passbolt/passbolt.php* or with the environment variable `APP_FULL_BASE_URL` if you are on Docker or Helm.
...
### TLS Certificate
These checks verify that the TLS certificate is valid and correctly configured to secure communications.
...
In order to trust the certificate on the server, you will need to create a `cert.crt` file which contain the chain of trust of the certificate (more information in [TLS troubleshooting](/hosting/tls/troubleshooting/)) and move this file in */usr/share/ca-certificates*
...
:::tip[PRO TIP]
If you are on Docker or Helm, you might want to double-check the values for `DATASOURCES_DEFAULT_*`. More information in the [environment variables reference](/hosting/environment-reference/)
:::
...
- Ensure that the driver defined for `Datasources.default.driver` in */etc/passbolt/passbolt.php*, or with the `DATASOURCES_DEFAULT_DRIVER` environment variable on Docker or Helm, is one of the following:
...
#### MariaDB/MySQL version deprecation
Since version 5.9.0, this warning-level check reports when the database engine runs a version passbolt plans to stop supporting: it warns below MariaDB 10.6 or MySQL 8.0. Plan a database engine upgrade to clear the warning.
...
You can also change the location of the keyring by editing the `GPG.env.setenv` and `GPG.env.home` variable in */etc/passbolt/passbolt.php*, or with the `GNUPGHOME` environment variable if you are on Docker or Helm.
...
- Ensure the public key file is defined by the variable `passbolt.gpg.serverKey.public` in */etc/passbolt/passbolt.php*, or by the `PASSBOLT_GPG_SERVER_KEY_PUBLIC` environment variable if you are on Docker or Helm.
...
- Ensure the private key file is defined by the variable `passbolt.gpg.serverKey.private` in */etc/passbolt/passbolt.php*, or by the `PASSBOLT_GPG_SERVER_KEY_PRIVATE` environment variable if you are on Docker or Helm.
...
#### Database schema
It checks that no database migration is pending. On failure, it reports "The database schema is not up to date." together with the command to run:
...
If the pending migrations come from a partially applied update, follow the [update guide](/hosting/update/).

#### Force TLS (SSL)
We recommend using TLS (SSL) with passbolt, this checks verify if passbolt has been configured to force TLS (SSL). If you have this error, you will need to set `passbolt.ssl.force` to true in */etc/passbolt/passbolt.php*, or the `PASSBOLT_SSL_FORCE` environment variable to true if you are on Docker or Helm.

<Figure
  src={
    '/img/help/2024/06/force-ssl.png'
  }
  caption="fig. Add forcing TLS (SSL) to the passbolt configuration file"
  alt="Add forcing TLS (SSL) to the passbolt configuration file"
/>
...
Ensure that the search engine robots are configured to not index the content, if it is not the case, set `passbolt.meta.robots` to false in */etc/passbolt/passbolt.php*, or the `PASSBOLT_META_ROBOTS` environment variable if you are on Docker or Helm.
...
Enabled by default, it check if the self registration plugin is enabled as expected. You can disable the plugin in */etc/passbolt/passbolt.php* through `passbolt.plugins.selfRegistration.enabled` and set it to false, or with the `PASSBOLT_PLUGINS_SELF_REGISTRATION_ENABLED` environment variable if you are on Docker or Helm.
...
It will ensure that the compiled version of the application is served, you should not face any errors, however if there is any, you should set `passbolt.js.build` to production in */etc/passbolt/passbolt.php*, or with the `PASSBOLT_JS_BUILD` environment variable if you are on Docker or Helm, otherwise passbolt will be slower.
...
### Directory Sync
It verifies that the endpoints for synchronising user directory configurations are enabled and functioning.

This is needed in order to configure LDAP through the user interface in *Administration > Users Directory* instead of using */etc/passbolt/ldap.php* which is only recommended if you are using multiple LDAP servers to synchronise the trees structure.
...
### SSO
This help with the TLS (SSL) certification validation for the Single Sign-On (SSO) instance is enabled and properly configured. There is a few things to consider if you are manually disabling it.
- Disabling the TLS (SSL) verification check can lead to security attacks.
...
<Figure
  ...
  caption="fig. Enable TLS (SSL) certificate validation for SSO"
  alt="Enable TLS (SSL) certificate validation for SSO"
/>

### JWT Authentication
These checks verify the JWT authentication setup, which is required for the mobile and desktop apps and the JWT API.

#### JWT Authentication plugin
This warning-level check verifies that the JWT Authentication plugin is enabled. A disabled plugin is reported as "The JWT Authentication plugin is disabled."

#### JWT directory not writable
The */etc/passbolt/jwt/* directory must not be writable by the webserver user. The check fails with "The /etc/passbolt/jwt/ directory should not be writable." Fix the permissions:
...
#### Valid JWT key pair
It ensures that a valid JWT key pair is present, otherwise the check fails with "A valid JWT key pair is missing." Create the key pair:

<CodeBlock>
  {`sudo su -s /bin/bash -c "/usr/share/php/passbolt/bin/cake passbolt create_jwt_keys" www-data`}
</CodeBlock>

See the [JWT key pair generation guide](/hosting/openpgp/generate-keys-jwt/) for the full procedure.

### Metadata
These checks verify the encrypted metadata keys used by the v5 resource format. The active metadata key, server access and key validity checks were introduced in version 5.4.0. The decryption check is older, version 4.10.0, where it was added as preparatory work before v5 metadata became generally available, and the zero-knowledge check was added in version 5.7.0.

:::note
An instance that does not use v5 metadata still reports most of these checks, and it reports them as passing rather than as skipped. The active metadata key, decryption and validity checks each display a green line, and the server access check has no v5 condition at all, so it runs whatever metadata format is in use. Only the zero-knowledge check is reported as skipped.
:::

#### Active metadata key
At least one active metadata key must exist, otherwise the check fails with "No active metadata key found." Create or activate a metadata key from the [encrypted metadata administration](/admin/resource-types/metadata-key/).

#### Server access to the metadata key
When zero-knowledge mode is off, the server must hold a copy of the metadata private key, otherwise it cannot share it with users. The check fails with "The server does not have access to metadata key."

#### Server metadata private key decryption
The server copy of the metadata private key must be decryptable with the server GPG key, otherwise the check fails with "Unable to decrypt the metadata private key." A classic cause is a server GPG key that was changed or rotated after the metadata key was shared.

#### Server metadata private key validity
The decrypted key must also be structurally valid, otherwise the check fails with "The server metadata private key is not valid."

For the access, decryption and validity checks alike, the remedy is an administrator sharing the metadata key with the server again, from an administrator's client: the server cannot grant itself access.

#### Zero-knowledge mode
The reverse check: when zero-knowledge mode is enabled, the server must not have access to the metadata private key. The check fails with "The server has access to the server metadata private key while in Zero-knowledge mode." Remove the server access, or disable zero-knowledge mode if it is not intended.

Zero-knowledge mode also changes the four checks above: all four are skipped once it is enabled. This check is skipped when v5 metadata is disabled, and skipped again when zero-knowledge sharing is disabled, so it appears in the report only when both are enabled. See the [encrypted metadata documentation](/admin/resource-types/encrypted-metadata/).
