...
The */etc/passbolt/jwt/* directory must not be writable by the webserver user. A too permissive setup is precisely what makes this check fail with "The /etc/passbolt/jwt/ directory should not be writable." Fix the permissions:
...
### Metadata
Since version 5.4.0, these checks verify the encrypted metadata keys used by the v5 resource format. They only run when v5 metadata is enabled, so a v4 instance reports nothing here.
...
#### Server metadata private key decryption and validity
The server copy of the metadata private key must be decryptable with the server GPG key and structurally valid, otherwise these checks fail with "Unable to decrypt the metadata private key." or "The server metadata private key is not valid." A classic cause is a server GPG key that was changed or rotated after the metadata key was shared.

For these access, decryption and validity checks, the remedy is an administrator sharing the metadata key with the server again, from an administrator's client: the server cannot grant itself access.

#### Zero-knowledge mode
The reverse check: when zero-knowledge mode is enabled, the server must not have access to the metadata private key. The check fails with "The server has access to the server metadata private key while in Zero-knowledge mode." Remove the server access, or disable zero-knowledge mode if it is not intended. See the [encrypted metadata documentation](/admin/resource-types/encrypted-metadata/).
