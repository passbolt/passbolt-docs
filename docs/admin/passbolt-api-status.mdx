...
If the check fails, it means that NTP could be inactive, and your system clock may not be synchronized. You can take a look at our [dedicated documentation](/docs/hosting/ntp/) for that.
### Config Files
:::tip[PRO TIP]
...
In order to trust the certificate on the server, you will need to create a `cert.crt` file which contain the chain of trust of the certificate (more information available [here](/hosting/tls/troubleshooting/)) and move this file in */usr/share/ca-certificates*
...
:::tip[PRO TIP]
If you are on Docker or Helm, you might want to double-check the values for `DATASOURCES_DEFAULT_*`. More information available [here](/docs/hosting/environment-reference/)
:::
