...
If a notification is missing on a given day, mind the trigger's granularity: `bin/cron` only fires the command during the 08:00 minute, so a custom schedule that never runs during that minute never sends it, and an email digest run lasting past 08:00:59 skips that day.
