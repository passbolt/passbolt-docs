...
1. **Open the console of the browser extension**, the one of its background worker rather than the one of the tab, and reproduce the loading of the list. The failure is logged there with the identifier of the resource, as "Unable to decrypt the metadata of the resource (`<id>`) using the user key.", or its shared key equivalent, which also names the key. A message mentioning a session key is benign: passbolt falls back to the normal decryption and discards the stale session key.
...
:::caution[No command can list them for you]
