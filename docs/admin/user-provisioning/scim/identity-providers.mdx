...
- Passbolt SCIM enabled and configured (see [SCIM Setup Guide](/admin/user-provisioning/scim/))
...
- **Authentication**: Bearer token (format: `pb_[A-Za-z0-9_-]{43}`)
...
| **Email** | `emails[type eq "work"].value` | Primary email address (the entry must be typed `work`) |

:::important[Email must be typed "work"]
Passbolt reads the account email only from the `emails` entry whose `type` is `work`. Mapping the email by array position (`emails[0].value`) is rejected with `400 Bad Request` ("No email with type work was found") when the first entry is not typed `work`, and the user is not created. Always map the email to `emails[type eq "work"].value`.
:::
...
Test your configuration with a small number of users before enabling for your entire organisation.
