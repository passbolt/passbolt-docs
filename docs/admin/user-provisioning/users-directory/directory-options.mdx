...
Mind the membership attribute: by default, the search targets `groupOfUniqueNames` groups but reads members from the `member` attribute, so the membership of standard `groupOfUniqueNames` groups is not picked up until the mapping is overridden. For directories using `groupOfUniqueNames` (the default object class), override the group users field mapping to `uniqueMember`, with the "Group users field mapping" field of the Users Directory screen or with `fieldsMapping.openldap.group.users` in the [file-based configuration](/admin/user-provisioning/users-directory/advanced-directory-options/#file-based-configuration). Change the **group object class** only if your groups use a different class, such as `groupOfNames`, and set the users mapping to match that class's membership attribute (`member` for `groupOfNames`).
