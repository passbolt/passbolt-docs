...
* Requirements:
  ...
  * Each group must list its members in the attribute passbolt maps as the group users field. The default mapping reads `member`, while `groupOfUniqueNames` groups store members in `uniqueMember`: see the mapping note in the example configuration below.
...
Mind the membership attribute: by default, the search targets `groupOfUniqueNames` groups but reads members from the `member` attribute, so the membership of standard `groupOfUniqueNames` groups is ignored until the mapping is overridden. Read the **Group object class** field before you change anything: `groupOfUniqueNames` is the default for a fresh setting, but an instance whose group object class was left empty and saved again may hold `groupOfNames` instead, and there the shipped `member` mapping is already correct. When the field shows `groupOfUniqueNames`, override the group users field mapping to `uniqueMember`, with the "Group users field mapping" field of the Users Directory screen or with `fieldsMapping.openldap.group.users` in the [file-based configuration](/admin/user-provisioning/users-directory/advanced-directory-options/#file-based-configuration). Change the **group object class** only if your groups use a different class, and set the users mapping to match that class's membership attribute (`member` for `groupOfNames`).
...
This can be changed with the "User username field mapping" field of the Users Directory administration screen, or with the `fieldsMapping` key of the [file-based configuration](/admin/user-provisioning/users-directory/advanced-directory-options/#file-based-configuration). Only the overridden key needs to be declared, the rest of the mapping keeps its defaults:
...
- Default mapping: `mail` maps to the Passbolt username
- Optional override: `userPrincipalName`, `sAMAccountName`
- Override with the "User username field mapping" field of the interface, or in the file-based configuration:
  ```php
  'fieldsMapping' => [
      'ad' => [
          'user' => [
  ...
  <tbody>
    ...
    <tr>
     ...
     <tr>
         ...
         <td>
         "Basic" (username and password, the default) or "SASL (EE)". With SASL, the username and password fields are hidden and not required: the bind uses the Kerberos credentials of the system user running passbolt (GSSAPI). Available for both directory types since version 4.0.0.
         </td>
         <td>
         <code>Basic</code>
         </td>
     ...
     <tr>
         ...
         <em>(required with Basic)</em>
         </td>
         <td>
         Username and password to authentify on your directory. Not used with the SASL authentication method.
         </td>
         <td>
         </td>
     </tr>
     ...
     <tr>
      ...
      <td>
        ...
        <tr>
            ...
            <td>
            Choose here the username of the passbolt admin user that will be used to perform the operations on behalf of the synchronisation tools.
        ...
        <tr>
            ...
            <td>
            Only for AD. Synchronise only the users that are enabled (=not disabled).
            </td>
            <td>
            </td>
        </tr>
      <tr>
           ...
           <td>
           By default, the synchronisation will be done for all created / deleted users and groups in your directory and all edited group members. You can enable / disable some tasks here.
