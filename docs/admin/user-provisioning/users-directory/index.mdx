---
title: Synchronise Users and Groups from Directory
description: How to Synchronise Users and Groups from a Directory
...
# How to Synchronise Users and Groups from a Directory
...
:::caution[Important]
The LDAP connector will send an invitation email to all the users matching your configuration during a synchronisation.
If you are simply testing it, make sure not to perform an actual synchronisation (use simulate sync instead), or
disable the cron job to send emails first.
:::


Passbolt Pro offers a user directory synchronisation feature (LDAP connector) that allows administrators to synchronise users and groups from an external directory into Passbolt.
...
<div className="info-box">
  ...
  <ul>
    ...
    <li><strong>Admin user and group manager</strong>: Assign a <strong>Default admin</strong> for performing syncs and a <strong>Default group admin</strong> for managing new groups.</li>
  </ul>
</div>
...
3. **Enter LDAP connection settings**
   ...
   - **Server URL**: `ldap://198.163.0.1:389` (unencrypted or with STARTTLS) or `ldaps://ldap.example.com:636` (implicit TLS)
...
5. **Save and test**  
   - Click **Save settings**.  
   - Run **Simulate synchronise** to preview changes.  
   - Review for expected users/groups and issues before a real sync.

6. **Run first synchronisation**  
   - Once verified, click **Synchronise**.  
...
Once the settings have been saved, the buttons "Simulate synchronise" and "Synchronise" at the top have become clickable.

Before we actually do a real synchronisation, we will first simulate one. Click on "Simulate synchronise" and wait
...
In this report, you will be able to see what will actually happen when you will synchronise your directory for real.
You will also be able to take corrective measures before an error actually happens.

### First synchronisation

To do the first synchronisation, repeat the same process as above. Only, click on "Synchronise" this time. A similar
report to the one that was displayed during a simulate will appear and let you know what happened exactly.

### User synchronisation example workflow

When an user is created in LDAP, they are imported in Passbolt using synchronisation.

If you delete this user in Passbolt, he will remain present in LDAP but won't be added back to Passbolt on next
synchronisation.

If you want to re-sync this user with LDAP, manually re-create him in Passbolt then run synchronisation. Passbolt
synchronisation tool will automatically recreate the link in Passbolt database.

If you delete this user in LDAP, he will be deleted from Passbolt on next synchronisation.

## Going further
### How to synchronise my directory automatically?
If you want to automate the synchronisation, you (or the person in charge of hosting) will need to [setup a cron job](/hosting/ldap/ldap-cron/).

### Configure LDAPS (implicit TLS)

If your configuration doesn't run out of the box with LDAPS, you can refer to the [LDAP with TLS (SSL) configuration](/hosting/ldap/ldaps/)
