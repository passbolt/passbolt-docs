---
title: Most common LDAP Synchronization Error Messages
description: Most Common LDAP Synchronization Error Messages
sidebar_label: LDAP Sync Errors
sidebar_position: 2
...
This error happens when a user was created first in Passbolt and later in the directory. Passbolt then considers that the passbolt user has the priority and should not be synced, since it would also mean that the same user would get deleted whenever it is deleted from the directory.
...
## The user user@domain.com could not be added to the group MyGroup because membership was removed in passbolt

This error happens when an user has been manually removed from a group. Passbolt then considers that the actions performed in passbolt have a higher priority and that the user was removed from the group for a good reason.

When this situation happens, the solution would be to manually re-add the user to the group in passbolt and then run the LDAP synchronization afterwards.
