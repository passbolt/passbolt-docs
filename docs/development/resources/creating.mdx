...
We then encrypt this data with our own public key, and sign it with our private key.
...
In this example, we sent the request to `/resources.json?api-version=v2&contain[permission]=1&contain[favorite]=1` so the answer will contain all the relevant information needed for displaying the newly-created resource.
