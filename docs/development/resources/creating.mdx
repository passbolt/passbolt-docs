...
The payload built below sends `name` and `uri` in clear, which only works for a resource type whose metadata is not encrypted. To create a resource in the v5 format, those fields go into an encrypted `metadata` field instead, as described in [working with v5 encrypted metadata](/development/resources/v5-encrypted-metadata/).
:::
