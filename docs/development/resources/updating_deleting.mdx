...
For this article, let's say that our secret, once decrypted, contains the following data:
...
We, as a client, would like to update the `username` field to "ada@passbolt.com",
...
The secret cannot be decrypted by the Passbolt API, so we need to update it as a whole, the decrypted secret looks like this:
...
Which is then encrypted using our own key, resulting in an armored string starting with `-----BEGIN PGP MESSAGE-----`.

> As this is a personal resource, we only need to re-encrypt our secret.
> If it was a shared one, we would need to re-encrypt the updated secret data for everyone having access to this resource, using their corresponding public key.

We need to tell the API which user is concerned by this secret, so the base payload is something like that:
...
As the metadata isn't End-to-end encrypted (yet 😉) an incremental update can be done.
...
In order to delete this resource, which has `fc11887b-aaa6-4600-a460-e05e99c75bce` as its ID,
