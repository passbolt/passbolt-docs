...
Let's say we want to find a resource we created, and we named it "NextCloud".
...
The final request looks something like this: `/resources.json?contain[resource-type]=1&contain[secret]=1`.

The server replies something that looks like this:
...
The request was a success, as indicated by the `status` field in the header, and the API returned an array with the all the resources we created in [another part of this guide](../creating).

We can now decrypt the secret associated to our account ID, `8bb80df5-700c-48ce-b568-85a60fc3c8f2`. 🎉

Now that we know our resource's ID, we can query information about it through a more specific endpoint, by making a `GET` request to `/resources/fc11887b-aaa6-4600-a460-e05e99c75bce.json`.
This way, we don't need to manipulate unneeded resources and save some CPU cycles for both our client and our server.

Congratulations, you just queried resources! 🎉
