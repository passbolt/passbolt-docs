...
:::info[This guide describes the v4 format]
The payloads below carry `name`, `username` and `uri` in clear, which is how a resource looks when its metadata is not encrypted. On an instance using the v5 format, those fields are absent from the response and their content is inside an encrypted `metadata` field instead. See [working with v5 encrypted metadata](/development/resources/v5-encrypted-metadata/).
:::
