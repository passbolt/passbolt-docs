...
<Incrementor
  title={props.productName === 'PRO'
    ? "Download our docker-compose-pro.yaml example file"
    : "Download our docker-compose-ce.yaml example file"
  }
  resetCounter={true}
/>
...
<div>
  <>
    {props.productName === 'PRO' && (
      <>
      <Incrementor title="Mount the subscription key or use the dedicated environment variable" />
        <p> Regarding the subscription key, you can either mount the subscription key (default) or use the dedicated SUBSCRIPTION_KEY environment variable.</p>
      <p>It's important to note that either mounting the subscription key or using the <code>SUBSCRIPTION_KEY</code> environment variable has to be set before running the container. <br /> If you want to mount the subscription key you can create in the same directory than the docker-compose-pro.yaml file a <code>subscription_key.txt</code> with the subscription key content. <br /> With the environment variable you will need to paste the base64-encoded subscription key content that has been sent from our sales team. Please replace the placeholder below with your own subscription key content:</p>
<CodeBlock>{`(..)
 passbolt:
  (...)
  environment:
    SUBSCRIPTION_KEY: "YOUR_SUBSSCRIPTION_KEY_HERE"
    (...)
  (...)`}</CodeBlock>
      </>
    )}
...
| EMAIL_DEFAULT_FROM               | From email address             | `'you@localhost.test'` |
...
[environment variable reference](/hosting/environment-reference/)

:::caution[Using a passbolt.php configuration file on Docker]
At container start, passbolt detects the fingerprint of its server GPG key automatically, but only when the `PASSBOLT_GPG_SERVER_KEY_FINGERPRINT` environment variable is not set **and** no `/etc/passbolt/passbolt.php` file exists. Mounting a `passbolt.php` file into the container disables this detection: if neither the file nor the environment defines the fingerprint, passbolt cannot find its server key and the instance stops working. If you mount a `passbolt.php`, always set `PASSBOLT_GPG_SERVER_KEY_FINGERPRINT` explicitly in your compose file.
:::
...
<>
  ...
  <ul>
    <li>
      <Link to="/docs/hosting/tls/pro/docker-auto/">Configure HTTPS for Docker with Let's Encrypt</Link>
    </li>
    <li>
      <Link to="/docs/hosting/tls/pro/docker-manual/">Configure HTTPS for Docker with a self-signed certificate</Link>
    </li>
  </ul>
  ) : (
  <ul>
    <li>
      <Link to="/docs/hosting/tls/ce/docker-auto/">Configure HTTPS for Docker with Let's Encrypt</Link>
    </li>
    <li>
      <Link to="/docs/hosting/tls/ce/docker-manual/">Configure HTTPS for Docker with a self-signed certificate</Link>
    </li>
  </ul>
  )}
</>
