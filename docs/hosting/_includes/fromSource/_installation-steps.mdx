...
:::caution
We highly recommend that you install https on your server. You can get a free TLS (SSL) certificate with the Let's Encrypt initiative.
...
- [Auto (Using Let's Encrypt)](/hosting/tls/ce/debian-auto/)
- [Manual (Using user-provided TLS (SSL) certificates)](/hosting/tls/ce/debian-manual/)

Be sure to write down the full path to your certificate and its corresponding private key, it will be needed later in the Nginx configuration process.
