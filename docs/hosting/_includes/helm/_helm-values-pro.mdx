As the `values.yaml` file is set up for CE by default, you need to point the passbolt image to a Pro tag. The tag lives under the `app.image.tag` key of `values.yaml`; replace the whole default value, including its pinned digest, with a [Pro tag](https://hub.docker.com/r/passbolt/passbolt/tags):
