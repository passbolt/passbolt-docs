...
This page is intended to give you the resources to set up NTP(or suitable equivalent) on the main distributions that we support. NTP is important for two main reasons with Passbolt. The first is in regards to GPG authentication. The other area where this becomes important is if you have MFA enabled as if the server and user device time get out of sync the codes will not work.
## Ubuntu

[Official Ubuntu Documentation](https://ubuntu.com/server/docs/use-timedatectl-and-timesyncd)

Ubuntu uses chrony for time synchronisation. This package is not installed by default so you'll need to install it.
...
To check if chrony is synchronised, make use of the tracking command:
```bash
chronyc tracking
```
...
Start the Network Time Protocol service manually
  -  Synchronise without Daemon

Set the system time periodically without a permanently running Network Time Protocol service. You can set the Interval of the Synchronisation in Minutes.
  -  Now and on boot
...
After you clicked Add to add a new time server in the address field, type the URL of the time server or pool of time servers with which you want to synchronise the machine time (for example, europe.pool.ntp.org).

After URL is complete, click on Test to verify that it points to a valid time source.

You can active Quick initial Sync to speed up the time synchronisation by sending more request at the Network Time Protocol service start or you can active Start Offline to speed up the boot time on systems that start the Network Time Protocol service automatically and may not have an internet connection at boot time.
...
chkconfig ntpd on
```
You can check upstream synchronisation with the ntpq command:
