...
<div>
  ...
  <ul>
    <li>
      <Link to={`/docs/hosting/tls/${props.productLabel.toLowerCase()}/debian-auto/`}>Auto (Using Let's Encrypt)</Link>
    </li>
    <li>
      <Link to={`/docs/hosting/tls/${props.productLabel.toLowerCase()}/debian-manual`}>Manual (Using user-provided TLS (SSL) certificates)</Link>
    </li>
  </ul>
</div>
