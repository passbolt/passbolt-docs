...
<>
  ...
  <p>
    ...
    licence you have to accept that in order to use this image by just clicking
...
<ul>
  <li>
    <Link to={`/docs/hosting/tls/${props.productName.toLowerCase()}/aws-auto/`}> Auto (Using Let's Encrypt) </Link>
  </li>
  <li>
    <Link to={`/docs/hosting/tls/${props.productName.toLowerCase()}/debian-manual/`}> Manual (Using user-provided TLS (SSL) certificates)</Link>
  </li>
</ul>
