...
<pre className="prism-code language-bash">
  <code>
    wget{' '}
    {`https://github.com/passbolt/passbolt-dep-scripts/releases/latest/download/passbolt-${props.productName.toLowerCase()}-SHA512SUM.txt`}
  </code>
</pre>
...
<pre className="prism-code language-bash">
  <code>
    sudo {props.distributionPackage} install passbolt-
    {props.productName.toLowerCase()}-server
  </code>
</pre>
