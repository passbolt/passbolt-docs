    ...
    {(() => {
      ...
      switch (props.distributionLabel) {
        case 'Digital Ocean':
          return (
            <>
              <p>Depending on your needs there are two different options to set up nginx and TLS (SSL):</p>
              <ul>
                <li>
                  <Link to={`/docs/hosting/tls/ce/digital-ocean-auto/`}>
                    Auto (Using Let's Encrypt)
                  </Link>
                </li>
                <li>
                  <Link to={`/docs/hosting/tls/ce/debian-manual/`}>
                    Manual (Using user-provided TLS (SSL) certificates)
                  </Link>
                </li>
              </ul>
            </>
        ...
        case 'RPM':
          return (
            <>
              <p>Depending on your needs there are two different options to setup nginx and TLS (SSL):</p>
              <ul>
                <li>
                  <Link to={`/docs/hosting/tls/${props.productLabel.toLowerCase()}/rpm-auto/`}>
                    Auto (Using Let's Encrypt)
                  </Link>
                </li>
                <li>
                  <Link to={`/docs/hosting/tls/${props.productLabel.toLowerCase()}/rpm-manual/`}>
                    Manual (Using user-provided TLS (SSL) certificates)
                  </Link>
                </li>
              </ul>
        ...
        case 'Debian':
          return (
            <>
              ...
              <ul>
                <li>
                  <Link to={`/docs/hosting/tls/${props.productLabel.toLowerCase()}/debian-auto/`}>
                    Auto (Using Let's Encrypt)
                  </Link>
                </li>
                <li>
                  <Link to={`/docs/hosting/tls/${props.productLabel.toLowerCase()}/debian-manual/`}>
                    Manual (Using user-provided TLS (SSL) certificates)
                  </Link>
                </li>
              </ul>
          ...
          return (
            <>
              ...
              <ul>
                <li>
                  <Link to={`/docs/hosting/tls/pro/ova-auto/`}>
                    Auto (Using Let's Encrypt)
                  </Link>
                </li>
                <li>
                  <Link to={`/docs/hosting/tls/pro/debian-manual/`}>
                    Manual (Using user-provided TLS (SSL) certificates)
                  </Link>
                </li>
              </ul>
        ...
        default:
          return (
            <>
              <p>Depending on your needs there are two different options to setup nginx and TLS (SSL) using the package:</p>
              <ul>
                <li>
                  <Link to={`/docs/hosting/tls/${props.productLabel.toLowerCase()}/debian-auto/`}>
                    Auto (Using Let's Encrypt)
...
<Figure
  src="/img/help/2026/06/webInstaller_configuration.png"
...
<Figure
  src="/img/help/2026/06/webInstaller_healthCheck.png"
  alt="Passbolt welcome page before configuration"
  caption="fig. Passbolt welcome page before configuration"
/>
  ...
  props.distributionLabel === 'Digital Ocean' ? (
    <>
      ...
      {props.productLabel === 'CE' ? (
        <Figure
          src={
            '/img/help/2026/06/webInstaller_DB.png'
          }
...
<>
  {props.productLabel === 'CE' ? (
    <Figure
      src={
        '/img/help/2026/06/webInstaller_GPG.png'
      }
...
<>
  {props.productLabel === 'CE' ? (
    <Figure
      src={
        '/img/help/2026/06/webInstaller_importGPG.png'
      }
...
<>
  {props.productLabel === 'CE' ? (
    <Figure
      src={
        '/img/help/2026/06/webInstaller_SMTP.png'
      }
...
<>
  {props.productLabel === 'CE' ? (
    <Figure
      src={
        '/img/help/2026/07/webInstaller_fullBaseUrl.png'
      }
...
<>
  {props.productLabel === 'CE' ? (
    <Figure
      src={
        '/img/help/2026/06/webInstaller_createUser.png'
      }
...
<>
  {props.productLabel === 'CE' ? (
    <>
      <p>At this step, the wizard will ask you for your subscription key. If you want to get started with the CE edition for now, you can simply skip this step. You can upgrade at any time later from your Organization Settings.</p>
      <Figure
        src={'/img/help/2026/06/webInstaller_CE-subscription.png'}
        alt={'First user creation from the Wizard'}
        caption={'fig. Wizard - First user creation'}
      />
    </>
    ...
    <>
      <p>At this step, the wizard will ask you for your subscription key. You should have received it by email soon after your online purchase. Enter it in the box.</p>
      <Figure
        src={'/img/help/2026/06/webInstaller_PRO-subscription.png'}
        alt={'First user creation from the Wizard'}
        caption={'fig. Wizard - First user creation'}
      />
    </>
...
That's it. The wizard will soon create your user account. You will see a redirection page for a few second and then will be redirected to the user setup process so that you can configure your user account.

<>
  {props.productLabel === 'CE' ? (
    <Figure
      src={
        '/img/help/2026/06/webInstaller_configurationComplete.png'
      }
