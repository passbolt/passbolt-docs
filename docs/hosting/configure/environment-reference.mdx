...
System environment variables override PHP configuration files but are overridden by web UI settings. These are ideal for infrastructure configuration and automation.
...
**Why This Priority:** Environment variables allow configuration without file changes, making them perfect for containerized deployments and infrastructure-as-code.
...
**Why This Priority:** Configuration files provide persistent, version-controlled settings that are ideal for traditional server deployments.
...
**Why This Priority:** Defaults ensure Passbolt works out-of-the-box while allowing complete customization through higher-priority methods.
...
For comprehensive TLS setup guidance, see [TLS Certificates](/hosting/configure/tls/). This section covers environment variables for outbound TLS verification.
:::
...
The following variables are critical for production security and should be properly configured in all deployments.
:::

**`PASSBOLT_SSL_FORCE`** *(boolean, optional)*  
Force SSL/HTTPS only. When enabled, all HTTP requests are redirected to HTTPS, ensuring all communication is encrypted. Critical for production security.  
**Default:** `false` | **Environment:** all
...
**`PASSBOLT_PLUGINS_SMTP_SETTINGS_SECURITY_SSL_VERIFY_PEER`** *(boolean, optional)*  
Verify SMTP server TLS (SSL) certificates during email transmission. Critical for secure email delivery.  
**Default:** `true` | **Environment:** all
...
Enable password expiry policy enforcement. Forces users to change their passwords after a specified period.  
**Default:** `true` | **Environment:** all
...
Enable password policy validation checks. Enforces password complexity requirements and validation rules.  
**Default:** `true` | **Environment:** all
...
Enable user passphrase policies. Controls the complexity requirements for user GPG passphrases.  
**Default:** `true` | **Environment:** all
...
Enable case-sensitive LDAP filters for directory synchronization. When enabled, LDAP search filters are case-sensitive, which may be required for some directory servers.  
**Default:** `false` | **Environment:** all
