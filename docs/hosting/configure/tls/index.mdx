...
- **SMTPS (Email)**: TLS secures email communications for notifications and user invitations. passbolt recommends Implicit TLS (SMTPS, port 465) and supports Explicit TLS (SMTP+StartTLS, port 587) depending on your SMTP server requirements. The SMTP server's certificate must be trusted by passbolt for TLS connections to succeed. For detailed SMTP TLS configuration including certificate trust settings, see [Email Server Configuration](/admin/emails/email-server/).
- **LDAPS**: Directory synchronization uses TLS to secure connections to Active Directory or other LDAP servers, see [LDAPS Configuration](/hosting/configure/ldap/ldaps/)
...
- [LDAPS Configuration](/hosting/configure/ldap/ldaps/): LDAP SSL configuration and certificate bundling
