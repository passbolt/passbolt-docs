---
title: Installing Passbolt in Non-Interactive Mode
...
| passbolt/nginx-certificate-file            | string   | Absolute path to TLS (SSL) certificate path (applies only if nginx-configuration-*-choices is manual)       |
| passbolt/nginx-certificate-key-file        | string   | Absolute path to TLS (SSL) key path (applies only if nginx-configuration-*-choices is manual)               |
