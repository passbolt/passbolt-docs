---
title: Using Docker Rootless Images
description: How to use Docker rootless images
sidebar_label: Rootless Images
...
services:
  ...
  passbolt:
    ...
    ports:
      ...
      - 443:4433
```
non-root images also uses a different path to handle TLS (SSL) certificates:
```yaml
version: '3.7'
services:
  db:
...
```
You can know more about how to setup https on docker on the [https configuration section](/hosting/tls/https/).


## LDAP
To use the LDAP cronjob on the non-root docker the following steps are needed:
- Create a new passbolt-pro-server cron file with an additional cron job
- The file by default has the email cronjob, so you must preserve that
