...
To use **LDAP with TLS (SSL)**, your LDAP server must present a valid TLS certificate to the client (in this case, the **Passbolt server**). The certificate must also be **trusted by Passbolt**.
...
> 💡 Passbolt provides an official [certificate bundler tool](https://github.com/passbolt/passbolt-ldap-certificate-bundler) to simplify retrieving and formatting certificates from LDAP servers with TLS.
...
These TLS options belong in `passbolt.php` or environment variables. They are separate from the file-based synchronisation settings (domains, filters), which live in `/etc/passbolt/ldap.php`; see the [file-based configuration reference](/admin/user-provisioning/users-directory/advanced-directory-options/#file-based-configuration).
:::
...
- sync behaviour involving deleted users or group permissions,
