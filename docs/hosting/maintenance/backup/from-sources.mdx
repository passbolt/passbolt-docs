---
title: Backup a Source Installation of passbolt
description: Backing up a passbolt installation from source
...
import DockerMariadbClientWarning from '/docs/hosting/_includes/warning/_docker-mariadb-client-warning.mdx';
import RestoreDatabaseNotes from '/docs/hosting/_includes/backup/_restore-database-notes.mdx';

# Backing up a passbolt installation from source
...
We made a dedicated command in order to make a backup of the database. Under the hood, it uses the dump tool that matches your database engine and version (mariadb-dump, mysqldump, or pg_dump), but we recommend using the passbolt command as it has been made to avoid any pasting or login details errors.

__Replace `WEB_SERVER_USER` with the correct one__. Depending on your OS, it could be nginx, www-data, etc.

<CodeBlock>
  {`sudo su -s /bin/bash -c "/usr/share/php/passbolt/bin/cake passbolt sql_export" WEB_SERVER_USER`}
</CodeBlock>

<DockerMariadbClientWarning />
...
The counterpart of `sql_export` is `mysql_import`, which loads an SQL backup back into the database. __Replace `WEB_SERVER_USER` with the correct one__:

<CodeBlock>
  {`sudo su -s /bin/bash -c "/usr/share/php/passbolt/bin/cake passbolt mysql_import --dir /path/to/backups --file passbolt-backup.sql" WEB_SERVER_USER`}
</CodeBlock>
