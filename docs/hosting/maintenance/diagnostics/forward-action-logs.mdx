...
```json
{"timestamp":"2026-08-05 09:12:33","user":"ada@passbolt.com","action":"password_access","context":"Ada Lovelace","status":1,"resource_id":"b8c4...","resource_name":"Production database","resource_username":"dbadmin","resource_uri":"https://db.example.com"}
```

It covers a fixed list of actions, emitted under these labels: `password_access`, `password_add`, `password_update`, `password_delete`, `user_login`, `user_logout` and `share`. Sharing actions carry a readable summary of the recipients and the permission granted.
