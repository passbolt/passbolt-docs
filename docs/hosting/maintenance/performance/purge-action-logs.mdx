...
:::tip[Shipping logs elsewhere changes the retention question]
If you forward action logs to a SIEM, the audit history is retained outside passbolt, so the database only needs to hold what your instance itself requires. That usually allows a shorter retention period here. Confirm your SIEM is ingesting successfully before you shorten it, because purging is irreversible and does not re-emit anything. See [forwarding action logs to a SIEM](/hosting/maintenance/diagnostics/forward-action-logs/).
:::
