...
The durations offered in the **remember me** dropdown come from the `passbolt.plugins.rememberMe.options` setting. It is set in `passbolt.php`: there is no environment variable and no administration screen for it. The defaults are:
...
[...]`}
  ...
  <TabItem value="docker" label="Docker">
    There is no environment variable for this setting, so the configuration file has to be mounted into the container:
