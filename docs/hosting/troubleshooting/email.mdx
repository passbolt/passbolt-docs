...
You can use `show_queued_emails` passbolt command to display email queue entries from the database. This is useful when there is any issue sending emails.
...
By default, the latest 15 records from the `email_queue` table will be displayed. However, it is also possible to filter records, see all the available options below:

* `--limit` = Number of records to show (Range: min 1 & max 100). Default is 15.
* `--failed` = Return only error records.
