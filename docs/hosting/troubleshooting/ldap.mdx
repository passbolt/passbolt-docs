...
<details>
  ...
  <p>This error happens when an user has been manually removed from a group. Passbolt then considers that the actions performed in passbolt have a higher priority and that the user was removed from the group for a good reason.</p>
  <p>When this situation happens, the solution would be to manually re-add the user to the group in passbolt and then run the LDAP synchronization afterwards.</p>
</details>
