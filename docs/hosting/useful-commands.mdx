...
For the full procedure, including the restore-specific precautions and the command for your installation type, see [backup and recovery](/hosting/maintenance/backup/).
...
<Tabs groupId="installation">
  <TabItem value="package" label="Package Installation">
    <CodeBlock>
      {`sudo su -s /bin/bash -c "/usr/share/php/passbolt/bin/cake passbolt edition_downgrade --username admin@example.com" www-data`}
    </CodeBlock>
  </TabItem>
  <TabItem value="source" label="From Source">
    <CodeBlock>
      {`sudo su -s /bin/bash -c "/var/www/passbolt/bin/cake passbolt edition_downgrade --username admin@example.com" www-data`}
    </CodeBlock>
  </TabItem>
  <TabItem value="docker" label="Docker">
    <CodeBlock>
      {`su -s /bin/bash -c "source /etc/environment && ./bin/cake passbolt edition_downgrade --username admin@example.com" www-data`}
    </CodeBlock>
  </TabItem>
</Tabs>

:::info
The same downgrade is available from the **Subscription** page in the administration panel.
See [updating your subscription key](/admin/subscription/update-subscription-key/).
:::
