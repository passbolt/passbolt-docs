...
To add several tags in one go, separate them with commas: typing `aws,prod,eu` creates three tags. A comma and the Enter key both commit the tag you are currently typing, so either works. Note that this applies to typing; pasting a comma-separated list does not split it.
