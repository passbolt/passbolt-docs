---
title: Troubleshooting Passbolt Windows Application
...
#### Let's Encrypt
There is usually no server issues with a certificate issued by a root CA (here, Let's Encrypt), you should be able to upload the account kit and proceed further without any issues.
...
- Browse the folder path
  - Trusted Root Certificate Authorities (CA)
- Click on "Finish"
...
- The folder path is wrong when installing the certificate
  - The certificate should be imported in **Trusted Root Certificate Authorities (CA)**
...
You can read more in [Microsoft's MSIX troubleshooting guide](https://learn.microsoft.com/en-us/windows/msix/msix-troubleshooting-guide).
