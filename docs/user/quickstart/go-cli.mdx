...
All channels except `go install` provide the command as `passbolt`, which is what this guide uses. Community packages also exist, see [Repology](https://repology.org/project/passbolt-cli/versions) for what your distribution offers.
...
Commands follow the same structure: `passbolt action entity`. The actions are `create`, `get`, `list`, `update`, `delete`, `move` and `share`, applied to `resource`, `folder`, `user` or `group` (`move` and `share` apply to resources and folders). In passbolt, a password is called a resource.
...
Only TOTP is supported, through the `mfaMode` setting:
